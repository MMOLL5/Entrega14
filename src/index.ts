...
let prod = new Producto("hola", 25, "asd", 1);
...
app.get('/listar/:id', (req, res) => {
    const vid: number = parseInt(req.params.id);
    const itemId = prod.listarItem(productos, vid);
...
app.post('/guardar/', (req, res) => {
    ...
    productos.push(prod);// = prod.guardar(productos);
...
app.put('/actualizar/:id', (req, res) => {
    ...
    const vid: number = parseInt(req.params.id)
    const actItem = prod.actualizar(productos, vid, body.title, body.price, body.thumbnail);
...
app.delete('/borrar/:id', (req, res) => {
    const vid: number = parseInt(req.params.id);
    const borrItem = prod.borrar(productos, vid);
...
let messages: { tit: string; pri: string; thu: string; }[] = [];
